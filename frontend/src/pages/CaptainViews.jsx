...
export const SquadList = () => {
    ...
    return (
        <div className="view-container fade-in">
            ...
            <div className="glass-table-container">
                <table className="glass-table">
                    <thead><tr><th>Name</th><th>Age</th><th>Gender</th><th>Status</th></tr></thead>
                    <tbody>
                        {players.length === 0 ? (
                            <tr><td colSpan="4" style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>No players found in your squad.</td></tr>
                        ) : players.map(p => (
                            <tr key={p.player_id}>
                                <td>{p.name}</td>
                                <td>{p.age}</td>
